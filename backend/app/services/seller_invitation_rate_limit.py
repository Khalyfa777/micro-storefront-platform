...
InvitationRateLimitAction = Literal[
    "validate",
    "accept",
]
...
def _rate_limit_key(
    ...
    client_hash = _ip_hash(
        _client_ip(request)
    )

    return (
        "rate-limit:seller-invitation:"
        f"{client_hash}"
    )
...
def _raise_rate_limit_error(
    ...
    raise HTTPException(
        ...
        detail=(
            ...
            f"Try again in {safe_wait_seconds} seconds."
        ),
        headers={
            "Retry-After": str(safe_wait_seconds),
        },
    )


async def _log_fallback_mode(
    ...
    async with _fallback_log_lock:
        ...
        logger.critical(
            "Redis is unavailable for seller invitation "
            "rate limiting. Using bounded in-process "
            "fallback. action=%s",
            action,
        )
...
async def _enforce_fallback_limit(
    ...
    window_seconds = (
        settings
        .SELLER_INVITATION_RATE_LIMIT_WINDOW_SECONDS
    )
    attempt_limit = (
        settings
        .SELLER_INVITATION_RATE_LIMIT_ATTEMPTS
    )
    ...
    async with _fallback_lock:
        ...
        if bucket is None:
            ...
            bucket.attempts += 1
            _fallback_buckets.move_to_end(key)
...
async def enforce_seller_invitation_rate_limit(
    request: Request,
    action: InvitationRateLimitAction,
) -> None:
    ...
    try:
        ...
        if (
            ...
            _raise_rate_limit_error(ttl)

    except HTTPException:
        raise

    except Exception:
        await _log_fallback_mode(action)
        await _enforce_fallback_limit(key)
