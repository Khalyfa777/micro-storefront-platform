...
async def _log_fallback_mode() -> None:
    ...
    async with _fallback_log_lock:
        ...
        logger.critical(
            "Redis is unavailable for public "
            "order tracking rate limiting. "
            "Using bounded in-process fallback."
        )
...
async def _enforce_fallback_limit(
    ...
    async with _fallback_lock:
        ...
        if bucket.attempts > TRACKING_LIMIT:
...
async def enforce_order_tracking_rate_limit(
    request: Request,
) -> None:
